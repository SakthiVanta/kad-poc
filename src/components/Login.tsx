...
export const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-white overflow-hidden">
      {/* Left/Top Side - Branding & Carousel */}
      <div className="w-full h-[25vh] md:h-screen md:w-[45%] lg:w-[40%] bg-primary relative p-8 md:p-12 flex flex-col items-center justify-center md:justify-between overflow-hidden rounded-b-[3rem] md:rounded-b-none md:rounded-r-[4rem] shadow-[0_20px_40px_-15px_rgba(var(--color-primary)/0.3)] md:shadow-[20px_0_60px_-15px_rgba(var(--color-primary)/0.3)] z-50 transition-all duration-500">
        {/* Background Pattern/Texture */}
        <div className="absolute inset-0 bg-gradient-to-br from-primary via-primary to-primary-container opacity-90" />
        
        {/* Animated Background Circles */}
        <div className="absolute -top-24 -left-24 w-64 h-64 md:w-96 md:h-96 bg-white/5 rounded-full blur-3xl" />
        <div className="absolute -bottom-24 -right-24 w-64 h-64 md:w-96 md:h-96 bg-white/5 rounded-full blur-3xl" />

        {/* Branding Header */}
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="flex items-center gap-3 mb-2 md:mb-4">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-white rounded-lg md:rounded-xl flex items-center justify-center p-1.5 md:p-2 shadow-lg">
          ...
          <p className="text-white/70 text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase">
            Field Intelligence
          </p>
        </div>

        {/* Carousel Content (Hidden on Mobile) */}
        <div className="hidden md:flex relative z-10 w-full max-w-2xl flex-col items-center text-center h-[320px] justify-center px-4">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <h2 className="text-2xl md:text-6xl font-bold text-white mb-2 md:mb-8 leading-tight tracking-tight">
                {carouselItems[currentSlide].title}
              </h2>
              <p className="text-white/80 text-sm md:text-xl leading-relaxed max-w-lg line-clamp-2 md:line-clamp-none">
        ...
        <div className="hidden md:flex relative z-10 justify-center gap-2 md:gap-4 w-full">
          {carouselItems.map((_, index) => (
            <button
              key={index}
              onClick={() => setCurrentSlide(index)}
              className={`h-1 md:h-2 rounded-full transition-all duration-500 ${
                index === currentSlide ? 'w-8 md:w-12 bg-white shadow-[0_0_15px_rgba(255,255,255,0.5)]' : 'w-1 md:w-2 bg-white/30 hover:bg-white/50'
              }`}
            />
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col p-8 md:p-16 lg:p-24 bg-surface dark:bg-surface-container-lowest transition-colors h-[60vh] md:h-screen overflow-y-auto">
        {/* Main Content Area (Form) - Centered Vertically */}
        <div className="flex-1 flex items-center justify-center w-full">
          <div className="w-full max-w-md">
            <div className="mb-10 text-center md:text-left">
              <h2 className="text-3xl font-bold text-on-surface mb-3">KAD CRM Login</h2>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-widest block ml-1">
                  Username
                </label>
              ...
              {error && (
                ...
                >
                  <p className="text-primary text-[10px] font-bold uppercase tracking-wider leading-tight flex items-center gap-2">
        ...
        <div className="pt-12 w-full max-w-4xl flex flex-col md:flex-row justify-between items-center gap-4 border-t border-outline-variant/10">
          <p className="text-[9px] font-bold text-on-surface-variant/40 uppercase tracking-widest text-center md:text-left">
            © 2024 BOOKWEDGO KAD CRM.
          </p>
          <div className="flex gap-6">
            <button className="text-[9px] font-bold text-on-surface-variant/40 hover:text-primary uppercase tracking-widest transition-colors">
              Privacy Policy
            </button>
            <button className="text-[9px] font-bold text-on-surface-variant/40 hover:text-primary uppercase tracking-widest transition-colors">
              Terms of Service
